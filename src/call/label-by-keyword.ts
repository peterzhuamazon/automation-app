...
//                       :          default to 'qwen2.5:3b' for 'ollama', and 'arn:aws:bedrock:us-east-1::inference-profile/us.anthropic.claude-3-5-haiku-20241022-v1:0' for 'bedrock'
    ...
    if (useLLM) {
      try {
        if (llmProviderProcess === 'ollama') {
          ...
          matchResponse = await bedrockInvoke(`${text}. ${llmPrompt}`, llmModelProcess);
